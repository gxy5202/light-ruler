 ...
 */
import { Ruler, WrapperSize, ResizeObject, Ratio, RulerOuterStyle, Params } from '../index.d';

import { MAX_SIZE, MAX_ROTIO, OVER_SIZE, DEFAULT_WIDTH, DEFAULT_HEIGHT } from './consts';

import getComputedStyle from '../utils/getComputedStyle';
import { renderDom, checkBrowserSupport } from '../utils/renderDom';
import OffscreenCanvasRuler from './OffscreenCanvasRuler';
...
export default class CanvasRuler {
    ...
    public canvas: OffscreenCanvasRuler | ScreenCanvasRuler; // canvas标尺实例 1.OffscreenCanvasRuler离屏canvas 2.主线程canvas
    ...
    public init(wrapId: string, options?: Ruler): void {
        ...
        this.options = {
            ...
            style: {
                size: style.size && style.size > MAX_SIZE ? MAX_SIZE : style.size || 20, // 标尺宽度: 横向标尺和纵向标尺相同
                backgroundColor: style.backgroundColor || '#171819',
                fontColor: style.fontColor || '#8c8c8c',
        ...
        if (this.ratio[0] > MAX_ROTIO || this.ratio[1] > MAX_ROTIO) {
    ...
    public createWrappedRuler(): void {
        const styleObject = {
            ...
            width: this.isInfinite ? this.wrapperSize.width + OVER_SIZE : this.options.contentWidth + OVER_SIZE, // 画布宽
            // eslint-disable-next-line max-len
            height: this.isInfinite ? this.wrapperSize.height + OVER_SIZE : this.options.contentWidth + OVER_SIZE, // 画布高
    ...
    public createCanvasInnerRuler(): void | never {
        ...
        // 选择最优渲染模式
        if (this.options.mode === 'auto' || this.options.mode === 'offscreen') {
            // 检测方法浏览器兼容性，进行canvas离屏渲染
            if (checkBrowserSupport(window.OffscreenCanvas) && checkBrowserSupport(Worker)) {
                this.canvas = new OffscreenCanvasRuler(params);
            } else {
                // 若浏览器不支持离屏渲染，则进行主线程渲染
                this.canvas = new ScreenCanvasRuler(params);
            }
        } else if (this.options.mode === 'screen') {
            this.canvas = new ScreenCanvasRuler(params);
        } else {
            throwCustomError('render mode is illegal');
        }
    }
    ...
    public resize(resizeObj: ResizeObject): CanvasRuler | never {
        if (typeof resizeObj !== 'object' || (!resizeObj.width && !resizeObj.height)) {
            throwCustomError('resize number is illegal');
        }

        let { width, height } = resizeObj;
        let size: number = resizeObj.size ? resizeObj.size : this.options.style.size;
        ...
        if (width > MAX_ROTIO || height > MAX_ROTIO) {
            ...
            wrapperHeight = this.wrapperSize.height;
        ...
        const styleObject = {
            wrapperId: this.wrapperId, // 容器id
            rulerId: this.options.rulerId,
            width: this.isInfinite ? wrapperWidth + OVER_SIZE : width + OVER_SIZE,
            height: this.isInfinite ? wrapperHeight + OVER_SIZE : height + OVER_SIZE,
            isInfinite: this.isInfinite,
            ...this.options.style
        };
    ...
    public updateUnitStyle(): void {
        ...
        this.unitDom.innerText = unit.text;
    }
    ...
    public destroy(): void {
        ...
        // 清空dom
        const cavasBox: HTMLElement = document.getElementById(`canvas-ruler-wrapper${this.options.rulerId}`);
        cavasBox.innerHTML = '';
        cavasBox.remove();
        // 释放内存
        this.wrapperId = null;
        ...
        this.canvasXbox = null;
        this.canvasYbox = null;
        this.options = null;
        this.canvas = null;
        this.ratio = null;
    ...
    public getEventNeedsDom(): void {
        this.canvasWrapper = document.getElementById(`canvas-ruler-wrapper${this.options.rulerId}`);
        this.scrollObject = document.querySelector(this.options.scrollSelector);
        this.canvasXbox = document.getElementById(`canvas-ruler-h-box${this.options.rulerId}`);
        this.canvasYbox = document.getElementById(`canvas-ruler-v-box${this.options.rulerId}`);
        this.unitDom = document.getElementById(`canvas-ruler-unit${this.options.rulerId}`);
    }
    ...
    public changeScrollObject(scrollSelector: string): CanvasRuler | never {
        const dom: HTMLElement = document.querySelector(scrollSelector);
        if (dom) {
            // 解绑事件
            // eslint-disable-next-line no-unused-expressions
            this.isInfinite
            ...
            throwCustomError(`can not find the scrollObject by selector ${scrollSelector}`);
        }
        return this;
    }
